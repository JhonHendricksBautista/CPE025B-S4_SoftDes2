...
class Figure {
    ...
    sortPoints() {
        this.elements.points = this.elements.points.sort((p1, p2) =>
            p2.x - p1.x == 0 ? p2.y - p1.y : p2.x - p1.x
        );
    }
    ...
    cleanUp() {
        ...
        this.elements.points = this.elements.points.reduce(
            (p, c) =>
                Figure.isSamePoint(p[p.length - 1], c)
                    ? p
                    : (p.push(c), p),
            []
        );
        ...
        this.elements.lines = this.elements.lines.reduce(
            (p, c) =>
                Figure.isSameLine(p[p.length - 1], c)
                    ? p
                    : (p.push(c), p),
            []
        );
    ...
    addElements(elements = []) {
        ...
        this.elements.points = this.elements.points.concat(
            elements.filter(e => e.type === 'point')
        );

        this.elements.lines = this.elements.lines.concat(
            elements.filter(e => e.type === 'line')
        );
